...
const RiderMap = nextDynamic(
  () => import('@/components/rider-map').then((m) => ({ default: m.RiderMap })),
  {
    ssr: false,
    loading: () => (
      <div className="h-full w-full animate-pulse bg-hir-surface" aria-label="Se încarcă harta…" />
    ),
  },
);
...
export default async function DashboardHome() {
  ...
  return (
    <div className="relative -mx-4 -mt-6 -mb-24 h-[calc(100vh-3.5rem)] sm:-mx-6">
      ...
      <div className="pointer-events-none absolute left-3 top-3 z-10 max-w-[62%] rounded-2xl border border-hir-border bg-hir-bg/90 px-3 py-2.5 backdrop-blur">
        <p className="text-sm font-semibold text-hir-fg">
          Bună, {profile?.full_name?.split(' ')[0] ?? 'curier'}
        </p>
        <p className="mt-1 flex items-center gap-1.5 text-[11px] text-hir-muted-fg">
          <span
            aria-hidden
        ...
        <div className="absolute right-3 top-3 z-10 flex max-w-[55%] flex-col gap-2">
          {activeOrders.slice(0, 3).map((o, idx) => (
            <Link
              ...
              className={`flex items-center gap-2 rounded-xl border bg-hir-bg/90 px-3 py-2 text-xs font-medium text-hir-fg shadow-lg backdrop-blur hover:bg-hir-surface ${
                idx === 0
          ...
          {activeOrders.length > 3 ? (
            <Link
              ...
              className="rounded-xl border border-hir-border bg-hir-bg/85 px-3 py-1.5 text-center text-[11px] font-medium text-hir-fg backdrop-blur hover:bg-hir-surface"
            >
      ...
      {!isOnline ? (
        <div className="fixed inset-x-0 bottom-16 z-[1200] px-4 pb-4">
          <div className="mx-auto max-w-xl rounded-2xl border border-hir-border bg-hir-bg/95 p-4 shadow-2xl backdrop-blur">
