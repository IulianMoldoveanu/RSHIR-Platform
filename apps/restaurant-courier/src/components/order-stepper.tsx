...
export function OrderStepper({ status }: { status: string }) {
  ...
  return (
    <ol
      ...
      className="flex items-center gap-1.5 rounded-2xl border border-hir-border bg-hir-surface px-4 py-3 ring-1 ring-inset ring-hir-border/40"
    >
      {STEPS.map((step, idx) => {
        ...
        const circle = isDone
          ? 'bg-violet-500 text-white border-violet-500 shadow-sm shadow-violet-500/40'
          : isCurrent
            ? 'border-violet-400 text-violet-200 animate-pulse ring-2 ring-violet-500/30'
        ...
        return (
          ...
          >
            <span
              className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-[11px] font-bold tabular-nums ${circle}`}
              aria-hidden
            >
