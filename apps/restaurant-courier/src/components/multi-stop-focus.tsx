  ...
  return (
    <div className="pointer-events-auto mx-auto flex w-full max-w-md flex-col gap-2.5 rounded-2xl border border-violet-400/40 bg-zinc-950/95 p-3 shadow-2xl shadow-violet-500/20 ring-1 ring-violet-500/10 backdrop-blur">
      <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-violet-300">
        ...
        <span className="text-violet-300/60">·</span>
        <span className="tabular-nums">~{formatDistance(totalRemainingM)}</span>
      </div>

      {/* NEXT — primary CTA */}
      <Link
        href={`/dashboard/orders/${next.orderId}`}
        className="group flex items-center gap-3 rounded-xl bg-violet-500/15 px-3 py-2.5 ring-1 ring-violet-500/30 transition-all hover:-translate-y-px hover:bg-violet-500/25 hover:shadow-lg hover:shadow-violet-500/20 active:translate-y-0 focus-visible:outline-2 focus-visible:outline-violet-400 focus-visible:outline-offset-2"
      >
        <span
          aria-hidden
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-violet-500 text-white shadow-md shadow-violet-500/40"
        >
          {next.kind === 'pickup' ? (
        ...
        <ol className="flex items-center gap-1.5 overflow-x-auto text-[11px] text-zinc-400">
          {remaining.map((s, i) => (
            <li key={s.orderId} className="flex items-center gap-1.5 whitespace-nowrap">
              <span
                aria-hidden
                className="flex h-5 w-5 items-center justify-center rounded-full bg-violet-500/20 text-[10px] font-bold tabular-nums text-violet-200 ring-1 ring-inset ring-violet-500/30"
              >
                {i + 2}
              </span>
              <span>{s.label}</span>
