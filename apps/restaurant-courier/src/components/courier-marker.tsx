 ...
 *
 * Sursă: importat din aplicația HIR Curier standalone — single source of truth
 * pentru marker-ul de curier live (tracking page customer + dispatch admin).
 */
...
export default function CourierMarker({
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`bg_${uid}`} x1={CX} y1={CY - R} x2={CX} y2={CY + R} gradientUnits="userSpaceOnUse">
          <stop stopColor={c2} />
        ...
        <filter id={`sh_${uid}`} x="-70%" y="-20%" width="240%" height="180%">
          <feDropShadow dx="0" dy="6" stdDeviation="5" floodColor="#000" floodOpacity={on ? 0.42 : 0.26} />
        </filter>
      </defs>
      ...
      {showPulse && (
        <>
          <circle
            ...
            r="18"
            fill={c}
            style={{ transformOrigin: `${CX}px ${CY}px`, animation: 'hirPulse 2.2s ease-out infinite' }}
          />
          <circle
            cx={CX}
            cy={CY}
            r="18"
            fill={c2}
            style={{ transformOrigin: `${CX}px ${CY}px`, animation: 'hirPulse2 2.2s ease-out infinite .6s' }}
          />
        </>
      )}
      ...
      <circle cx={CX} cy={CY} r={R} fill="none" stroke="#fff" strokeOpacity={on ? 0.3 : 0.12} strokeWidth="1.5" />
...
/* ==================================================================
 * INTEGRARE MAPBOX GL — injectare ca marker custom
 * ==================================================================
 * npm i mapbox-gl
 *
 *   import { renderToStaticMarkup } from "react-dom/server";
 *   import mapboxgl from "mapbox-gl";
 *   import CourierMarker, { Vehicle, CourierStatus } from "./courier-marker";
 *
 *   type Courier = { id: string; lng: number; lat: number; vehicle: Vehicle; status: CourierStatus; heading: number; };
 *
 *   const markers = new Map<string, mapboxgl.Marker>();
 *
 *   function upsertCourier(map: mapboxgl.Map, c: Courier) {
 *     let m = markers.get(c.id);
 *     const el = document.createElement("div");
 *     el.innerHTML = renderToStaticMarkup(
 *       <CourierMarker vehicle={c.vehicle} status={c.status} heading={c.heading} animate={false} />
 *     );
 *     if (!m) {
 ...
 *   }
 *
 * Pe tracking client (1 curier) folosește animate={true}.
 * Pe dispecerat (multe markere) folosește animate={false}.
 * ================================================================== */
