...
import { calcRoi } from '../calculator-client';
...
describe('calcRoi — hepi toggle', () => {
  ...
  it('hepiRoi > 1 at default params', () => {
    const r = calcRoi(100, 80, false, true);
    expect(r.hepiRoi).toBeGreaterThan(1);
  });
});

describe('calcRoi — totals', () => {
  it('totalLuna = economieComisioane (no toggles)', () => {
    const r = calcRoi(100, 80, false, false);
...
describe('calcRoi — edge cases', () => {
  it('handles minimum slider values (10 comenzi/zi, 30 lei AOV)', () => {
    const r = calcRoi(10, 30, false, false);
    expect(r.comenziLuna).toBe(300);
    expect(r.venitBrut).toBe(9_000);
  ...
  it('handles maximum slider values (500 comenzi/zi, 200 lei AOV)', () => {
    ...
    expect(r.totalAn).toBe(r.totalLuna * 12);
  });
});
