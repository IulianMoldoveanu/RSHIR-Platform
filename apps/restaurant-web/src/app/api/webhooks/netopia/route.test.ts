...
//   - payment.captured triggers markOrderPaidAndDispatch
//   - payment.authorized triggers markOrderPaidAndDispatch
...
describe('POST /api/webhooks/netopia', () => {
  beforeEach(() => {
    ...
    pspPaymentsSelectMock.mockResolvedValue({ data: { order_id: ORDER_ID } });
  ...
  it('payment.captured → calls markOrderPaidAndDispatch with the order id', async () => {
    verifyWebhookMock.mockResolvedValue({
      kind: 'payment.captured',
      providerRef: PROVIDER_REF,
      amountBani: 5000,
