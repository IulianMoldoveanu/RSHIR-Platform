  ...
  if (pspRow) {
    ...
    try {
      if (event.kind === 'payment.captured') {
        // 2026-06-16 — TOCTOU fix: claim the row via CAS BEFORE dispatching.
        ...
        // webhook must not flip the ledger row back to CAPTURED (e.g. a
        // redelivered "captured" IPN arriving after a refund).
        if (
        ...
        if (!claim.data || claim.data.length === 0) {
          Sentry.addBreadcrumb({ category: 'webhook.viva', message: 'order.race_lost', level: 'info', data: { orderId: pspRow.orderId } });
          return NextResponse.json({ received: true, duplicate: true });
        }
      ...
      Sentry.captureException(err, {
        tags: {
          subsystem: 'webhook.viva',
          event_kind: event.kind,
          side_effect: 'failed',
        },
        extra: { orderId: pspRow.orderId, providerRef: event.providerRef },
        fingerprint: ['webhook.viva.side_effect_failed', event.kind],
      });
    }
  } else {
