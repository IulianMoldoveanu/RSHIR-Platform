// Lead capture for the /calculator-roi interactive ROI tool.
// Same pattern as /api/connect/lead: same-origin guard + rate limit +
...
// RO phone: 07XXXXXXXX or +407XXXXXXXX — simple sanity check, not strict.
...
const bodySchema = z.object({
  ...
  comenziPerZi: z.number().int().min(10).max(500),
  aovLei: z.number().int().min(30).max(200),
...
async function notifyTelegram(args: {
  ...
  const savings = new Intl.NumberFormat('ro-RO').format(args.estimatedSavingsMonthlyLei);
  const lines = [
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[calculator-leads] telegram failed', res.status, await res.text());
    }
  } catch (e) {
...
export async function POST(req: NextRequest) {
  ...
  const { phone, restaurantName, city, comenziPerZi, aovLei, estimatedSavingsMonthlyLei } =
    parsed.data;
