                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-3">
                        <FleetOfferDownloadButton fleetName={t.name} />
                        <button
                          type="button"
                          onClick={() =>
...
// ────────────────────────────────────────────────────────────
// Per-row "Download offer PDF" link. The PDF is generated by the
// restaurant-web app (api/fleet-offer-pdf) so it lives next to the
// existing react-pdf sales-sheet plumbing. WEB_URL falls back to the
// public production domain so the button keeps working in local dev
// when restaurant-web is not running on the same host.
// ────────────────────────────────────────────────────────────

function FleetOfferDownloadButton({ fleetName }: { fleetName: string }) {
  const base =
    process.env.NEXT_PUBLIC_RESTAURANT_WEB_URL ?? 'https://hirforyou.ro';
  const href = `${base.replace(/\/$/, '')}/api/fleet-offer-pdf?fleet=${encodeURIComponent(fleetName)}`;
  return (
    <a
      href={href}
      target="_blank"
      ...
      title={`Descarcă oferta PDF pentru ${fleetName}`}
      aria-label={`Descarcă oferta PDF pentru ${fleetName}`}
    >
      Descarcă oferta PDF
    </a>
  );
