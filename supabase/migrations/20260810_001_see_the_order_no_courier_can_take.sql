...
create or replace view public.live_ops_telemetry with (security_invoker = on) as
 WITH active_orders AS (
         SELECT o.tenant_id,
            ...
            -- Codex P1 (#1071): adding the courier alert was not enough — the
            -- same order stayed in this counter, so the kitchen kept being
            -- paged for a delay it did not cause. An order whose courier job
            -- is still unclaimed is not a slow kitchen, and is excluded here.
            count(*) FILTER (WHERE (o.status = ANY (ARRAY['PENDING'::text, 'CONFIRMED'::text, 'PREPARING'::text, 'READY'::text]))
                             AND o.created_at < (now() - '00:15:00'::interval)
                             AND NOT EXISTS (
                               SELECT 1 FROM courier_orders sco
                                WHERE sco.source_type = 'HIR_TENANT'
                                  AND sco.source_order_id = o.id::text
                                  AND sco.status = ANY (ARRAY['CREATED'::text, 'OFFERED'::text])
         ...
         SELECT co.source_tenant_id AS tenant_id,
            co.created_at,
            COALESCE(f.name, '(fără flotă)') AS fleet_name,
            NOT EXISTS (
         ...
         -- Codex P2 (#1071): the fleet name and the unstaffed flag must come
         -- from the same order. Aggregating them separately could name a
         -- staffed fleet (oldest order) while flagging unstaffed (any order),
         -- producing a CRIT that accuses the wrong fleet. Both picks below use
         -- an identical ORDER BY, so element [1] is one row: the unstaffed
         -- fleet first when there is one, oldest otherwise.
         SELECT tenant_id,
            count(*) AS courier_unclaimed_over_10m,
            (max(EXTRACT(epoch FROM (now() - created_at))) / 60::numeric)::integer AS courier_unclaimed_oldest_min,
            (array_agg(fleet_name ORDER BY unstaffed DESC, created_at))[1] AS courier_unclaimed_fleet,
            (array_agg(unstaffed ORDER BY unstaffed DESC, created_at))[1] AS courier_unclaimed_fleet_unstaffed
           FROM unclaimed_rows
          GROUP BY tenant_id
        )
